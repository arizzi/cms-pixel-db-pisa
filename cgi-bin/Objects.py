...
class Test_FullModule(object):
      ...
      TCYCLERROR = Float()
      COMMENT= Unicode()
      def __init__(self,FULLMODULE_ID,SESSION_ID,RESULT,ROCSWORSEPERCENT,NOISE,TRIMMING,PHCAL,CURRENT1UA,CURRENT2UA,CURRENT1501UA,CURRENT1502UA,IVSLOPE,TEMPVALUE,TEMPERROR,TCYCLVALUE,TCYCLERROR,COMMENT,FINALGRADE,GRADE,FULLTESTGRADE,SHORTTESTGRADE,DATA_ID=0):
	    ...
	    self.FULLTESTGRADE=unicode(FULLTESTGRADE)
            ...
            self.CURRENT1UA=float(CURRENT1UA)
            self.CURRENT2UA=float(CURRENT2UA)
            self.CURRENT1501UA=float(CURRENT1501UA)
            self.CURRENT1502UA=float(CURRENT1502UA)
